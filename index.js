...
const slackUrlRouter = {
    ...
    addMessage: function(message) {
        ...
        if (!this.messageQueues[slackUrl]) {
            ...
            const configProperties = ['username', 'servername', 'buffer', 'slack_url', 'channel', 'buffer_seconds', 'buffer_max_seconds', 'queue_max'];
            const config = {};
...
pm2.launchBus(function(err, bus) {
    ...
    if (moduleConfig.log) {
        bus.on('log:out', function(data) {
            if (data.process.name === 'pm2-slack-ous') { return; } // Ignore messages of own module.
